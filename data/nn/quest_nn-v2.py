...
def cli():
    ...
    parser.add_argument("--predict", type=str, help="Path to a JSON file with one or more excited states.")
    ...
    parser.add_argument("--deterministic", action="store_true", help="Use deterministic prediction (no MC dropout)")
    ...
    if args.train:
        if not args.data_dir:
            console.print("[bold red]❌ Please specify --data-dir.[/bold red]")
            sys.exit(1)
        ...
        for epoch in range(10000):
            ...
            if epoch % 50 == 0:
                ...
                with torch.no_grad():
                    ...
                    console.print(f"Epoch {epoch:4d}, Loss: {loss.item():.4f}, Val MAE: {val_mae:.4f}")
        ...
        for entry in sample_data:
            ...
            try:
                if args.deterministic:
                    mean = predict_deterministic(model, pipeline, entry)
                    std = 0.0
                else:
